# poslednata kolona ke ni sluzhi karakteristiki koi gi znaeme za toa mnozhestvo (dali kolata e prifatliva ili ne)
# informacija za klasata, treba da napravime nekoj model koj ke ja pretpostavuva
# taa soodvetna klasa
# prva redica - ime na karakteristkata
# poslednta kolona - pretstavva klasa

import csv  #modul koj se stpravva za citanje csv faljovi
from sklearn.naive_bayes import CategoricalNB
# za da mozheme da gi konvertirame vo numerichki vrednosti site kategoriski atributi
from sklearn.preprocessing import OrdinalEncoder #ke znaj da praj vid na konverzija small=0 medium=1 int.. kompatibilnost so modelot

def read_file(filename):
    with open(filename) as doc:
        csv_reader=csv.reader(doc,delimiter=',')
        data=list(csv_reader)[1:] #prajmelista koja ke pocni od vtoroto do krajot

    return data

if __name__=='__main__':
# ke treba da go vcitame kako lista od listi (mnozhestvoto)
# sekoja vnatresna lista se oodnesuva ne eden zapis

    dataset = read_file('car.csv')
    encoder=OrdinalEncoder()

#so kreiranje na promenliva ke funkcionita enkodero - treniranje na model?

    encoder.fit([row[:-1] for row in dataset]) #ke go trenirme so koi podatoci

#  podatocnoto mnozestvo go delime na povekje mnozestva
#  za prvite 70%

    train_set=dataset[:int(0.7* len(dataset))]

# za podelba ke gi smestime karakteristikite od prviot do predposledniot element

    train_x=[row[:-1]for row in train_set]

# i uste edna promenliva za klasata t.e. poslednata kolona

    train_y=[row[-1]for row in train_set]

# transformacija so enkoderot prethodno kade ima nekoja karakteristika
# string treba da semapira celobrojna vrednost, ke go predade kako vlez
# a ke vrati kopija na izlez
...
# trenranje na modelot

    classifier= CategoricalNB()
# ke treba da kazime sto ke bide vlez a sto izlez

    classifier.fit(train_x,train_y) #vkame fit samo s mnozestvoto na treniranje

    # ova go klavame kako proverka za prviot
    predicted_class=classifier.predict([test_x[0]])[0]
    true_class=test_y[0]

    accuracy=0

    for i in range(len(test_set)): #treba da vidime koja klasa e vistinska koja e predvidena
    ...
    print(f"Verojatnosti za pripadnost na klasite: {probabilities}")
