...
//*dataset-ot e najdolu na zadacata*
...
if __name__ == '__main__':
    ...
    #bez indeksot na kolonata
    str = [str[row] for row in range(len(str)) if row != kolona]
    dataset = [[row[red] for red in range(len(row)) if red != kolona] for row in dataset]
    ...
    for i in range(len(test_set)):
        predicted = klasifikator.predict([test_x[i]])[0]
    ...
    predicted = klasifikator.predict([str])[0]
    print(predicted)
    predictedProba = klasifikator.predict_proba([str])[0]
